...
lfbossa@gmail.com, 2016
    ...
    def _output(self,thing,source):
        ...
        if source=='definirproblema':
            ...
            if 'html' in self.output:
                texto = '<table> '+ htmlmatrix(self.c)  +'</table> x <br>'
                texto = texto + ' s. a. <table>' + htmlmatrix(self.A) + '</table> x = '
                bempe = htmlmatrix(self.b.reshape((len(self.b),1)))
                texto = texto + "<table>" + bempe + '</table>'
                self.html.write(texto)
        ...
        if 'minimal' in self.output:
            ...
            if 'html' in self.output  and '[m]' in source:
                if type(thing) == str:
                    self.html.write('<p>' + thing + '</p>')
                if type(thing) == np.ndarray:
                    matriz = htmlmatrix(thing)
                    self.html.write(' <table>'+ matriz +'</table>')
